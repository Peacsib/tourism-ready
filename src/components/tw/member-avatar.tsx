...
export function MemberAvatar({ m, size = "md", className }: { m?: Pick<Member, "full_name" | "avatar_url"> | null | undefined; size?: keyof typeof SIZES; className?: string }) {
  const s = SIZES[size];
  const [broken, setBroken] = useState(false);
  if (m?.avatar_url && !broken) return <img src={m.avatar_url} alt="" onError={() => setBroken(true)} className={cn(s, "shrink-0 rounded-full border object-cover", className)} />;
  return (
